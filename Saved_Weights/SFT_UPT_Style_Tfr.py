...
parser.add_argument("--generator_weights_path", type=str, default="/checkpoint/ajwaniro/Identity_GPT2_Checkpoints/gpt2-large_1e-05lr_32_bsz2epochs")   #Note: generator loaded directly as it was saved using HF trainer
...
def generate(batch_x):
    ...
    attention_mask_Flipped = torch.flip(attention_mask_withoutPrompt, dims=[-1])
    attention_mask_Output = attention_mask_Flipped[:,L-max_new_tokens:]    # as output length = L-2 
    ...
    for generate_step in range(max_new_tokens):
        ...
        if (generate_step == 0):
            ...
            outputs_withoutPrompt = generator_model(inputs_embeds=input_embeddings, attention_mask=attention_mask_withoutPrompt, past_key_values=past_key_values_withoutPrompt, return_dict=True)
        ...
        softmax_logits = torch.exp(nn.LogSoftmax(dim=1)(gumbel_logits_withPrompt/temperature))

        next_token_embeddings_generator = torch.matmul(softmax_logits, embeddings_matrix_generator)  # (bsz, D_gen)
        input_embeddings = next_token_embeddings_generator.unsqueeze(1)   #(bsz, 1, D_gen)

        next_token_embeddings_classifier = torch.matmul(softmax_logits, embeddings_matrix_classifier)  # (bsz, D_clf)
