...
project_name = 'StyleTfr SPT-UPT-PPP'
...
#classifier_checkpoint = "/checkpoint/ajwaniro/Classifier_Checkpoints_GPT2/SST_Dataset/gpt2_sentiment_2class_classifier_5epochs_3e-5_Cleaned.pt"

classifier_tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

classifier_model = AutoModelForSequenceClassification.from_pretrained('gpt2', num_labels=2, pad_token_id=classifier_tokenizer.eos_token_id).to(device)
...
def CategoricalCrossEntropy(output_logits, target_logits, attention_slice):  #output_logits are of shape [bsz,vocab], targets are of shape [bsz], attention_slice is of size [bsz]
...
def generate(batch_x):
    ...
    for generate_step in range(max_new_tokens):
        ...
        loss = CategoricalCrossEntropy(output_logits=logits_withPrompt , target_logits=logits_withoutPrompt, attention_slice=attention_mask_Output[:,generate_step])    # -------- Loss Component 1
...
for epoch in range(args.n_epochs):
    ...
    train_table.rows.append(["Input", "Output Greedy", "Output Non Prompted"])
    ...
    val_table.rows.append(["Input", "Output Greedy", "Output Non Prompted"])
