...
parser.add_argument("--classifier_model", type=str, default='gpt2-large')
...
if (args.desired_label==0):
    run_name = 'Pos2Neg_'+args.generator_model+'_'+args.classifier_model+'_'+str(args.prompt_length)+'prompt_'+str(args.fluency)+'fluency_'+str(args.learning_rate)+'lr_'+str(args.batch_size)+'bsz_'+str(args.n_epochs)+'epochs_'+str(args.train_data_size)+'_'+str(args.val_data_size)
else:
    run_name = 'Neg2Pos_'+args.generator_model+'_'+args.classifier_model+'_'+str(args.prompt_length)+'prompt_'+str(args.fluency)+'fluency_'+str(args.learning_rate)+'lr_'+str(args.batch_size)+'bsz_'+str(args.n_epochs)+'epochs_'+str(args.train_data_size)+'_'+str(args.val_data_size)
...
project_name = 'StyleTfr SPT-UPT PromptClf'
...
classifier_model = AutoModelForSequenceClassification.from_pretrained(args.classifier_model, num_labels=2, pad_token_id=classifier_tokenizer.eos_token_id)
print("Loaded Classifier Model")

classifier_model = classifier_model.to(device)

#chkpt_dir = '../../Yelp_Classifier/Prompt_Clf/GPT2_Large/Prompts/gpt2-large_100prompt_0.0001lr_32bsz_3eps'
chkpt_dir = '../../Yelp_Classifier/Prompt_Clf/GPT2_Large/Prompts/gpt2-large_20prompt_0.001lr_32bsz_3eps'

prompt_embeddings_clf = torch.load(chkpt_dir+'/prompts.pt', map_location=torch.device('cpu')).to(device)

ll_scores = torch.load(chkpt_dir+'/scores.pt', map_location=torch.device('cpu')).to(device)

classifier_model.score.weight = torch.nn.Parameter(ll_scores)
...
for epoch in range(args.n_epochs):
    ...
    for batch in train_dataloader:
        ...
        prompt_embeddings_clf_broadcasted = torch.stack([prompt_embeddings_clf]*bsz)

        attention_mask_output_withPrompt = torch.cat([torch.ones((bsz, prompt_embeddings_clf.shape[0])).to(device), attention_mask_Output], dim=1)  #-> attention mask for prompt + source

    #print("Input")
    #print(generator_tokenizer.batch_decode(input_ids, skip_special_tokens=True))

        classifier_output = classifier_model(inputs_embeds=torch.concat([prompt_embeddings_clf_broadcasted, classifier_embeddings], dim=1), attention_mask=attention_mask_output_withPrompt, labels=labels)
